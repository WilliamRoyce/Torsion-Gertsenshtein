...
| --- | --- | --- | --- |
| **M0** ✅ `654b627a` (2026-09-06) | **the scaffold** (this document's companion) plus `pyproject` second console script, extras, YAML package-data, the hygiene test, the CI lane | new lane green; no `import tidal` under the new root | the `drop` rows: `sweep`, `sample`, `analyze`, `plot` and their tests, once §5 records the decision — **recorded 2026-09-09, executed `df206443` (2026-09-11, #533); `measure` is drop + M5, not M0** |
| **M0.5** ✅ `e310e125` (2026-09-06; 185 fixtures, 46 pairs) → **197 fixtures, 49 pairs** `02b57ae3` (2026-09-12, #547: three of the four excluded theories derived; `theory_radial` measured un-derivable and excluded with its reason) | **freeze the oracle** — run legacy across the example corpus and commit the outputs. **Before any porting begins** | fixtures committed and reproducible | — |
