...
\end{lstlisting}
when running \texttt{tidal simulate} with a parameter set to
...
\textbf{Symptom:}
\begin{lstlisting}[style=python]
NotImplementedError: _build_evolution_matrices operates per k-mode and cannot
represent a position-dependent kinetic coefficient (field(s) ...) ...
\end{lstlisting}
from \texttt{\_build\_evolution\_matrices} (reached directly by the
conversion-stability probe or \texttt{modal\_jax}) or from
\texttt{solve\_modal\_pass1} (the per-mode Duhamel correction path).
...
kinetic term.  Since GH \#427, plain \texttt{solve\_modal} handles this: the
routing predicate sends such specs to the convolution paths, which fold the
per-grid-point $M^{-1}(x)$ into the real-space coefficients before the FFT
(mathematically identical to the mass-side $\hat{M}^{-1}(k-k')$ convolution).
Only the strictly per-mode engines still refuse, because a position-dependent
$M$ couples Fourier modes and no per-mode mass matrix exists: the
generalized-eigenvalue builder (probe/\texttt{modal\_jax} entry) and the Pass-1
Duhamel path.  Before the GH \#421 guards this failed as a grid-less
\texttt{evaluate\_coefficient} \texttt{ValueError} on some paths and was
\emph{silently treated as $M = 1$} on the generalized-eigenvalue path --- including
inside the inference stability probe.

\textbf{Fix (one of):}
\begin{itemize}
  \item Use plain modal: auto-selection (or \texttt{--scheme modal}) runs the
    spec on the convolution paths with full $M(x)$ support (GH \#427);
    validated against CVODE to $<1\%$ RMS on the Euler--Heisenberg
    dual-Gaussian spec.
  ...
  \item For perturbative flows, declare the small parameter in
    \texttt{[perturbation].small\_parameters}: kinetic canonicalization
    (GH \#380) moves the position dependence into $O(\varepsilon)$ RHS
    corrections and the base spec stays constant-coefficient (this is the
    Euler--Heisenberg dual-Gaussian flow).  Pass-1 correction kinetics that
    remain position-dependent are refused (per-mode Duhamel).
  \item A mode-coupling-aware stability gate for the inference probe is
    tracked in GH \#441; until it lands, localized-background specs cannot be
    stability-gated.
\end{itemize}
...
\subsubsection{SingularPencilError: ``Deflation contract violated'' / ``not simulatable as posed''}
\label{sec:troubleshooting-pencil-refusal}

\textbf{Symptom}: \texttt{tidal simulate} (modal) raises
\texttt{SingularPencilError} with ``Deflation contract violated \ldots
at determination floor $\tau$'', or the stability probe reports
\emph{not simulatable as posed}.

\textbf{Cause}: no double-precision evolution operator both satisfies the
equations (the contract of \cref{sec:pencil-engine-impl}) and is
integrable. Two known classes: (a) a genuine near-index breakdown at the
parameter point (nonminimal T4 class, GH \#467) --- the adaptive floor
resolves most of these and warns with the final $\tau$; (b) the localized
implicit-dynamical class (ungauged gravity on a position-dependent
background, GH \#468): the far-field background restores the gauge
freedom below machine precision and the quotient's row compression
discards independent equations (GH \#474).

\textbf{Fix}: for (b), excite only the sector you measure
(\texttt{--ic-component}, \texttt{--ic-field}); the run then restricts
itself to that sector's dependency closure when it is exactly closed
(\cref{sec:pencil-general-input}), writing \texttt{restricted\_spec.json}.
If the note says the excited sector couples into the implicit-dynamical
sector, there is at present \emph{no general float64 fix}: both attempted
routes are closed with measured evidence --- the Kronecker-like staircase
(GH \#473) and symbolic gauge-structure discovery (GH \#477), the latter
showing that the weakly determined directions carry about a fifth of the
true motion and so cannot be pinned away. Extended precision is the
surviving candidate (decision record GH \#470). Gauge fixing at
derivation (\cref{sec:gauge-fixing}) remains a per-theory option.
Never lower the contract tolerance --- the refusal replaces a silently
wrong trajectory (GH \#455).

\subsubsection{``field(s) \ldots were not evolved on this closure-restricted run''}
\label{sec:troubleshooting-closure-absent}

\textbf{Symptom}: \texttt{tidal measure} refuses total energy or a
conversion source/target on a run whose \texttt{metadata.json} points at
\texttt{restricted\_spec.json}.

\textbf{Cause}: the run evolved the observable-sector closure only;
fields outside it are absent from the outputs by design (never zero),
and Hamiltonian terms touching them were dropped
(\texttt{metadata.restriction} in the restricted spec records which).

\textbf{Fix}: measure within the evolved sector (conversion there is
exact), re-run exciting the field you need so it joins the closure, or
\texttt{--no-closure-restriction} to attempt the full system (which
refuses honestly on this class).

\subsubsection{Gauge certificate ``FLAGGED''}
\label{sec:troubleshooting-gauge-flagged}

\textbf{Symptom}: \texttt{tidal measure} prints ``Gauge certificate:
FLAGGED --- \ldots read pinned (gauge-quotiented) content''.

\textbf{Cause}: the engine pinned undetermined state directions to zero
(the explicit minimum-norm choice, see \cref{sec:pencil-quotient}) and
the requested measurement's field support overlaps them; the value
depends on that gauge choice.

\textbf{Fix}: this is physics information, not an error --- the quantity
is not an observable of the theory as posed. Measure a gauge-invariant
quantity (the certified conversion channel), or gauge-fix the theory at
derivation and re-derive; for spec-level redundancy (byte-identical
equation copies, GH \#465) re-derive the theory (GH \#374).
...
\subsubsection{Position-Dependent Theory at Periodic BCs Diverges (GH \#367)}

\textbf{Resolved in v0.41.6 (2026-05-19).} The historical symptom was
field-amplitude growth by $10^6$--$10^9\times$ and a
\texttt{SimulationDivergedError} from
\texttt{\_evolve\_full\_matrix}. If you are seeing this on a current
tip-of-tree build, you have either pinned a pre-v0.41.6 version of
\texttt{tidal/solver/modal.py} or hit a different bug — proceed to
diagnostics below.

\textbf{Root cause and fix}: \texttt{\_build\_convolution\_matrix}
was missing the $M^{-1}$ (inverse \texttt{kinetic\_coefficient\_symbolic})
folding that \texttt{\_build\_per\_mode\_matrices} already applied
(\#301/\#302). Theories whose graviton or torsion modes carry a
non-trivial kinetic coefficient (Gertsenshtein: $-1/\kappa^2$; massive
vector torsion: $-\xi$) had a discrete Laplacian with the wrong sign
at every $k$, giving the state matrix real eigenvalues $\pm k$ that
$\exp(A t)$ faithfully amplified. The visible $\max\Re(\lambda) =
k_{\max}$ was the symptom of a uniform-in-$k$ sign error, not a
non-normal pseudospectral artifact at the Nyquist edge. See
\S\ref{sec:modal-pos-dep-auto-route} for the full diagnosis and the
list of rejected hypotheses.

\textbf{Diagnostic checklist} if you are still hitting amplitude growth
on a current tip-of-tree build:
\begin{itemize}
  \item Confirm you are on v0.41.6 or later
    (\texttt{git log --oneline tidal/solver/modal.py | head -5}).
  \item Run \texttt{uv run python scripts/benchmarks/modal\_vs\_ida\_inprocess.py}
    — at $N \in \{64, 128\}$, modal should agree with CVODE on the
    h$_5$ peak to $<$1\% at $t_{\mathrm{end}} = 20$.
  \item If the disagreement is large, dump
    \texttt{kinetic\_coefficient\_symbolic} on the divergent field's
    equation — if it is non-trivial and the per-mode path also
    disagrees with CVODE, the bug is elsewhere.
  \item As a last-resort manual workaround: pass \texttt{--scheme
    cvode} (no constraints) or \texttt{--scheme ida} (with
    constraints — CVODE silently freezes those at IC).
\end{itemize>
