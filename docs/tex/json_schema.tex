...
\end{itemize}
This function is called automatically by \texttt{tidal simulate} after
parameter resolution; it is exported from
...
\subsection{Mass and Coupling Matrices (not stored)}
\label{sec:json-coupling}

The JSON contains \textbf{no} \texttt{coupling} section. Mass and coupling
matrices are fully determined by the \texttt{identity} operator terms already
present in \texttt{equations}, so storing them would duplicate data the reader
can derive.

They are available on the loaded model as
\texttt{EquationSystem.mass\_matrix}, \texttt{.coupling\_matrix} and their
\texttt{\_symbolic} counterparts, computed once by
\texttt{\_compute\_matrices\_from\_terms}.
...
This makes mass-squared positive for the standard Lagrangian sign convention:
$\ddt^2 \phi = \ldots - m^2 \phi$ produces $\texttt{mass\_matrix[i][i]} = m^2$.

\begin{itemize}
  \item Diagonal entries ($i = j$) go in \texttt{mass\_matrix}
  \item Off-diagonal entries ($i \neq j$) go in \texttt{coupling\_matrix}
  \item Contributions \textbf{accumulate}: a component may carry several
        \texttt{identity} terms acting on the same field, and the entry is
        their sum
  \item Neither matrix is normalised by the LHS kinetic coefficient; for an
        effective mass-squared divide by it, via
        \texttt{spec\_query.effective\_coefficient}
\end{itemize}

The numeric and symbolic forms differ deliberately: the numeric matrices apply
the negation above, while the symbolic ones hold
\texttt{coefficient\_symbolic} verbatim so it can be resolved at runtime
against real parameter values. An identity term \texttt{B0\^{}2/8} therefore
gives \texttt{mass\_matrix = -0.125} and
\texttt{mass\_matrix\_symbolic = "B0\^{}2/8"}.

\paragraph{History.}
\label{par:json-coupling-history}

Before \texttt{c407240d} (2026-02-07) the loader \emph{read}
\texttt{coupling.mass\_matrix} from the JSON and those values were
authoritative. That commit switched to deriving them from the equation terms,
but the exporter kept writing the section --- so a second, unread copy was
produced by a separate Wolfram implementation for six months, and the two
drifted apart without anything noticing (GH \#403, \#404). The export was
removed rather than repaired. Specs derived before that removal still carry a
\texttt{coupling} section; it is ignored.
