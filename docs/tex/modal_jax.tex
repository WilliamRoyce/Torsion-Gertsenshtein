...
This was validated on CSD3 sapphire by two short sampler chains (INTR QOS,
$N_{\mathrm{samples}}=50$).  The driver script was retired with the sampler
subcommand it invoked (GH \#533); it remains at tag \texttt{v0.53.0} as
\texttt{scripts/hpc\_jax\_benchmark.sh}.  Sapphire compute nodes (Intel
Sapphire Rapids, larger L2 cache, faster BLAS) showed better JAX speedup than
the devcontainer.
