...
\texttt{--output DIR}              & ---               & Directory for snapshot data (no extension; images are no longer rendered) \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Scanning a parameter}

The dedicated sweep subcommand was retired after \texttt{v0.53.0} (GH \#533);
parameter campaigns run through Cobaya's samplers.  A shell loop still covers
the simple case:

\begin{lstlisting}[style=bash]
for m2 in 0.5 1.0 2.0 4.0; do
    tidal simulate examples/data/coupled_scalars.json \
        --param mPhi2=$m2 --output "run_m2_${m2}"
done
\end{lstlisting}
