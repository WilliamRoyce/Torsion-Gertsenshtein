...
\textbf{Status:} Implemented (Monte Carlo + nested sampling via PolyChord) \\
\textbf{Date:} 2026-04-02

A fixed parameter grid scales exponentially with the number of parameters and
produces neither posterior distributions nor evidence estimates.  This module
adds a Bayesian inference layer: it draws samples from prior distributions,
...
\textbf{Status (2026-09-09).} The subcommand that drove this module was retired
after \texttt{v0.53.0} (GH \#533); under decision D9 inference moves into the
Cobaya ecosystem, which ships PolyChord itself.  What follows documents
\texttt{tidal.inference} as a \emph{Python library}, which is now the only way
it is reached.  The package retires at M1b, once the Cobaya \texttt{Theory}
class stands up.
...
Parallel evaluation uses \texttt{multiprocessing.Pool} with a picklable
module-level worker.
...
\midrule
\textbf{PolyChord} & (only backend) & GitHub + gfortran & Development + HPC & MPI + OpenMP \\
\bottomrule
\end{tabular}
\end{table}

\emph{Correction (2026-09-04).} Earlier revisions of this section documented a
second backend, \textbf{dynesty}, selected with \texttt{--sampler dynesty} and
installed by the \texttt{inference} extra.  \textbf{No such backend was ever
implemented.}  \texttt{dynesty} appears nowhere in \texttt{tidal/}, is not a
declared dependency (the \texttt{inference} extra is \texttt{anesthetic} alone),
and there is no \texttt{--sampler} flag --- \texttt{tidal/cli/\_sample.py} passes
\texttt{sampler="polychord"} unconditionally.  The references below to dynesty as
a fallback, a default, or an install target are corrected in place.  PolyChord is
the only nested-sampling backend.
...
posterior visualization.  It is backend-agnostic by design, so a second
backend could be added later without touching any analysis code.
...
The inference pipeline reuses the shared run-point stages with zero
duplication:

\begin{itemize}
  \item \textbf{Simulation:} \texttt{SimulationLikelihood} in
    \texttt{tidal/inference/\_likelihood.py} reaches \texttt{simulate\_run()},
    \texttt{run\_inference\_step()} and \texttt{measure\_run()} through
    \texttt{tidal/measurement/\_run\_stages.py}.  Those stages lived in
    \texttt{tidal/cli/\_sweep.py} until GH \#533 retired the sweep subcommand
    and moved them into \texttt{tidal/measurement/}.
  \item \textbf{Output:} \texttt{InferenceResult.to\_sweep\_results()} converts
    ...
    export.
  \item \textbf{Parallelism:} a picklable module-level worker is used for
    \texttt{Pool}-safe likelihood evaluation.
\end{itemize}

Install the optional inference dependencies:

\begin{lstlisting}[style=bash]
pip install "tidal[inference]"   # adds anesthetic >= 2.0
# PolyChord is installed separately (needs gfortran):
#   bash scripts/install_polychord.sh
\end{lstlisting}
...
\textbf{Usage.}  Both entry points were CLI flags on the retired
\texttt{sample} and \texttt{analyze} subcommands (GH \#533); the computation is
reached directly as \texttt{tidal.inference.\_importance}.
...
(dynesty/MultiNest) above $\sim$10 dimensions, which is why it is the backend
...
It is the only backend: no pure-Python fallback exists.

\subsubsection{Why anesthetic for visualization?}

anesthetic (Handley 2019)~\cite{handley2019anesthetic} reads PolyChord output
natively (and dynesty's, should a second backend ever be added).  Using it for
all visualization keeps the analysis layer backend-agnostic, so adding a sampler
later would require no change to plotting code.
...
\begin{itemize}
  ...
  \item \textbf{Speagle (2020)}~\cite{speagle2020dynesty}, ``dynesty: a dynamic
    ...
    \url{https://github.com/joshspeagle/dynesty} --- Cited for the algorithmic
    comparison in \S\ref{sec:inference-design}; \textbf{not} a TIDAL backend.
