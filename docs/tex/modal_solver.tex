...
Position-dependent coefficients are OK --- RHS coefficients $c(x)$ and
kinetic coefficients $M(x)$ alike --- because such specs route to the
convolution paths, which build the mode-coupling matrix in the full FFT
basis and fold $M^{-1}(x)$ into the coefficients in real space
(GH \#427/\#445; \cref{par:modal-posdep}).  The carve-out is internal, not
an eligibility criterion: the strictly per-mode engines (the
generalized-eigenvalue builder reached by the stability probe and
\texttt{modal\_jax}, and the Pass-1 Duhamel path) refuse a
position-dependent $M(x)$ because no per-$k$ mass matrix exists for it ---
see \cref{sec:troubleshooting-posdep-kinetic-modal}.  The full
...
\texttt{tests/test\_solver\_modal.py} & 83 tests (eligibility, correctness, stability, constraint, and the GH \#421/\#427 position-dependent-kinetic suites) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Superseded (August 2026, GH \#457/\#469).} The jerk substitution
below is retained as history. The pencil engine of \cref{sec:pencil-engine}
handles constraint chains of any index without it; jerk-level content on
singular-mass or promoted (implicit-dynamical) systems now \emph{refuses}
with an actionable error rather than executing the substitution, which
the audit found had never been exercised by any test and was wrong on the
affected $\tilde R^2$-class specs.
