...
removed in v0.50.0.  Keeping a switch for an abandoned policy leaves it
...
sweep path untouched, so for four months the sweep path silently
returned \texttt{run\_status="tachyonic"} with \texttt{wall\_time\_s
= 0} --- never simulating --- while the inference path recorded the
...
(\texttt{tidal.measurement.\_posthoc\_audit}; its \texttt{analyze} CLI
front-end was retired under GH \#533) remains available as a debug
