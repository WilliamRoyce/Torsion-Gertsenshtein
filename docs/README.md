...
| `evidence/` | Committed artifacts behind live decisions, one directory per run. `tier1-20260907/` holds the Tier-1 MISMATCH evidence for **#543**, read back **read-only** via `scripts/psalter/summarize_diff.py` — a recompute path would overwrite the verdict it is meant to report |
