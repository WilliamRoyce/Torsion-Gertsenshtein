...
ladder, the workstreams, and the **wave board**, which is the single source of truth for
current state. The rest are the research records behind it, produced by the **H-series**
research handoffs and, since Wave 0, the **I-series** implementation handoffs.

*(There are seven `H*.md` prompt files, not eight: H7 was executed inside the orchestrator's
own planning session rather than dispatched, so only its output — `spectator_route.md` — was
committed. `docs/cosmology/handoffs/README.md` indexes every prompt of both series.)*
...
| `stage1_measurements.md` | The Wave-0 record of what a **live PSALTer install** actually does: install route, wall times, the Tier-1 gate verdict, and probes #521–#523. Several design claims are overturned here — read it beside `stage1_engineering_plan.md`, not instead of it |
| `evidence/` | Committed artifacts behind live decisions, one directory per run. `tier1-20260907/` holds the Tier-1 MISMATCH evidence for **#543** and `tier1-20260911-pass/` the `VERDICT: MATCH` that resolved it (the cause: two undeclared Function Repository dependencies — `psalter_543_investigation.md`; the upstream write-up, unfiled: `psalter_543_upstream_issue.md`); read back **read-only** via `scripts/psalter/summarize_diff.py` — a recompute path would overwrite the verdict it is meant to report; `tier1-20260911-engine-142/` is the one-time 14.2.1 cross-check (MATCH; that engine is retired) |
| `handoffs/` | The prompt each session was given, H- and I-series, with **`handoffs/README.md`** as the index (issue, wave, Wolfram lane, status, outputs). Kept as the record of *what was asked*, which is what lets a reader judge whether the deliverable answered it |

Two directories outside `docs/cosmology/` belong to the programme as well:
...
## Comparison Records (`docs/comparison/`)

Per-configuration comparisons between survey revisions (`*_v2_v3.md`) — amplification and
suppression deltas for the D1, D20/Bahamonde, D21/Barker and Stage-A points. Campaign
bookkeeping, in the same class as the records above.

## Sphinx API Docs (`docs/source/`)

Auto-generated API documentation via Sphinx. Build with `make html` from `docs/`.

Only `index.rst` and `conf.py` are tracked — `docs/source/.gitignore` is `*.rst` with
`!index.rst`, so everything else is regenerated by `sphinx-apidoc` at build time. **Both
packages are documented**, and each needs its own table-of-contents file: `--tocfile` defaults
to `modules`, so a second bare invocation would overwrite the first's toctree and silently
drop that package from the published site (#545). `.github/workflows/deploy.yml` therefore
runs `sphinx-apidoc` twice — plain for `tidal/`, and `--tocfile modules_cosmo` for
`tidalcosmo/` — with both referenced from `index.rst`. Collapse this back to one invocation
at M7, when the rename makes a second package root unnecessary.
